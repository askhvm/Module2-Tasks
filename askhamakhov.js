...
// Обьект вида
// {
//     sortBy -  название поля по которому будет происходить сортировка [ в качестве поля для сортировки могут использоваться поля из основного обекта без вложенности ]
...
const searchTransactions = (arr, obj) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    filtered.push(arr[i]["split"]);
  }
  console.log(filtered);
  let equals = filtered.filter((e) =>
    e.some(
      (item) =>
        item[obj.year] &&
        item[obj.month] &&
        item.category.includes(obj.searchText)
    )
  );
  let sorted = equals.sort((a, b) =>
    obj.sortDir === "ask"
      ? a[obj.sortBy] - b[obj.sortBy]
      : b[obj.sortBy] - a[obj.sortBy]
  );
  return sorted;
};
...
const getTotal = (arr, obj) => {
    let splits = [];
    for (let i = 0; i < arr.length; i++) {
      splits.push(arr[i].split);
    }
  return splits.flat().reduce((acc, el) => (acc += el.absAmount), 0);;
};
searchTransactions(a, info);
getTotal(a, info);
// Имеется массив
//
// Создать функцию searchTransactions которая принимает первым параметром массив
