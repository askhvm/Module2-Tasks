...
const searchTransactions = (arr, obj) => {
  let sorted = arr.sort((a, b) =>
    obj.sort.charAt(0) !== "-"
      ? a[obj.sort] - b[obj.sort]
      : b[obj.sort] - a[obj.sort]
  );
  let categ = [];
  for (let i = 0; i < sorted.length; i++) {
    if (sorted[i].split) categ.push(sorted[i].split);
  }

  console.log(categ)
  let result = categ.map((e) => e.category + e.year + e.month);
  return result;
};
...
const getTotal = (arr, obj) => {
  let splits = arr.filter((e) => e = e.split);
  let result = "";
  for (let i = 0; i < splits.length; i++) {
    result += splits[i]["split"]["absAmount"];
  }
  return result;
};

console.log(searchTransactions(a, info));
console.log("_________________")
console.log(getTotal(a, info));
